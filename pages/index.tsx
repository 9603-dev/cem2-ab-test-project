      ...
      {state != 3 ? (
        <div className="flex flex-col items-center justify-center text-center w-80">
          <div className="text-zinc-800">
            คุณจะซื้อเมล็ดทานตะวันแพคเกจสี
            <span className={state ? 'text-red-600 font-bold' : 'font-bold'}> {color} </span>หรือไม่
          </div>
          <Image className="my-4 rounded-xl" src={pic} alt={''} width={250} height={37}></Image>
          <button className="btn btn-primary w-full my-2" onClick={buyBtnHandler}>
            ซื้อ
          </button>
          <button className="btn btn-primary w-full my-2" onClick={dontBuyBtnHandler}>
            ไม่ซื้อ
          </button>
        </div>
      ) : (
